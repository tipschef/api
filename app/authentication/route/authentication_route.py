...
async def authenticate_user(form_data: OAuth2PasswordRequestForm = Depends()):
    try:
        ...
        response = AuthenticationService.authenticate_user(user)
        return {"access_token": response.token, "token_type": response.token_type}
...
async def get_user_data(current_user: UserAuthSchema = Depends(UserService.get_current_active_user)):
